...
export default function InsertCompany() {
  ...
  const [isGenderOpen, setIsGenderOpen] = useState(false); // Estado correto para abrir o modal de gênero
  const [selectedCity, setSelectedCity] = useState<string>('');
  const [selectGender, setSelectedGender] = useState<string>(''); // Corrigido: useState correto para gênero
  const [selectedState, setSelectedState] = useState<string>('Rio de Janeiro');
  const [selectedTable, setSelectedTable] = useState<string>('eventos'); // Default table
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const catalogFile = formData.get('catalog') as File; // Adicionar o arquivo de catálogo

      if (logoFile) {
        data.logo = await toBase64(logoFile);
      }

      if (catalogFile) { // Converter o arquivo de catálogo para base64
      ...
      data.gender = selectGender; // Adiciona o gênero no envio de dados
  ...
  const openGenderModal = () => setIsGenderOpen(true); // Corrigido: Função para abrir o modal de gênero
  const closeGenderModal = () => setIsGenderOpen(false); // Corrigido: Função para fechar o modal de gênero
  ...
  const handleGenderSelect = (gender: string) => { // Corrigido: função para selecionar o gênero
    const genderValue = genderMap[gender as keyof typeof genderMap];
    if (genderValue) {
      setSelectedGender(genderValue);
      closeGenderModal(); // Corrigido: Fecha o modal de gênero após a seleção
    }
  };
  ...
  const displayGenderName = (value: string): string => {
    ...
    return entry ? entry[0] : 'Selecione o gênero do anúncio'; // Corrigido: Exibe o gênero selecionado
  };
      ...
      />
              ...
              <div className="mt-6 w-full flex justify-center">
                <Image
                  ...
                  alt='Teste Image'
                  priority
                  layout="intrinsic" // Adapta ao tamanho da imagem
                />
              </div>
            </div>
          </div>
